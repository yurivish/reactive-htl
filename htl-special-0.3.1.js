// A lightly modified version of version 0.3.1 https://github.com/observablehq/htl:
// https://github.com/observablehq/htl/blob/e3e9777d09f7b5aea3ebc52e73b86a1246dcab7e/src/index.js
// adding the ability to treat certain interpolations as special and process them separately.
// We use this in index.js to add reactivity through fine-grained effects, keeping signal-specific
// concerns separate from the core of the hypertext literal. This could in theory be upstreamed;
// I've kept all original code formatting, and the diff is small.
  ...
  return function({raw: strings}) {
    ...
    for (let j = 0, m = arguments.length; j < m; ++j) {
      ...
      if (j > 0) {
        ...
        const valueIsSpecial = isSpecial && isSpecial(value);
        switch (state) {
          case STATE_RAWTEXT: {
            if (value != null) {
              const text = `${value}`;
...
export function processAttrs(node, name, value) {
...
export function processAttr(node, name, value) {
...
export function processChildren(node, parent, value) {
...
export function setStyles(style, values) {
